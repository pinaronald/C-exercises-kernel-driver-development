 ...
 *       Filename:  graph.c
 *
 *    Description: v2. Write a set of parametirzed macro "clear_bit" 
 *    test_bit to go with set_bit operation    
 *    Draw a diagonal line across the graphics array and 
...
int main()
{
	...
	* */
    ...
    printf("\nPrint the bitmap after bit clear...\n");                                             
	print_graphics();                                                           

    printf("\nTest if the bit are clear...\n");                          
	for (loc=0; loc < X_SIZE; ++loc) {                                            
      if (TEST_BIT(loc, loc))
			printf ("Bit [%d,%d] is not clear.\n", loc, loc);
	}
		return (0);
